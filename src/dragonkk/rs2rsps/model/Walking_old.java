 ...
 * @author Graham
 */
public class Walking_old implements Serializable {
	...
	public Walking_old(Entity entity) {
		this.entity = entity;
		setWalkDir((byte) -1);
		setRunDir((byte) -1);
	...
	public void addStepToWalkingQueue(int x, int y) {
		int diffX = x - walkingQueue[wQueueWritePtr - 1].x, diffY = y
				- walkingQueue[wQueueWritePtr - 1].y;
		byte dir = (entity instanceof Npc) ? (byte)Misc.getNPCWalkDir(diffX, diffY): (byte) Misc.walkingDirection(diffX, diffY);
	...
	public void getNextEntityMovement() {
		...
		byte walkDir = (entity instanceof Player) /*&& System.currentTimeMillis() < p
				.getCombatDefinitions().getLastEmote() - 600)*/ ? getNextWalkingDirection()
