...
public class Misc {
	...
	public static String decompressHuffman(InStream packet, int numChars) {
		...
		packet.read(textBuffer, packet.remaining());
  ...
  public static String UncapitaloptimizeText(String text) {
   ...
   return new String(buf, 0, buf.length);
	...
	public static String decryptPlayerChat1(InStream str, int totalChars) {
		try {
			if (totalChars == 0) {
				return "";
			}
	...
	/*
	 * public static final byte[] WalkStepX = new byte[]{-1, 0, 1, -1, 1, -1, 0,
	 * 1}; public static final byte[] WalkStepY = new byte[]{-1, -1, -1, 0, 0,
	 * 1, 1, 1};
	 * 
	 * public static void WalkDir(int MoveX, int MoveY, int walkDir) { if
	 * (walkDir == 0) { MoveY--; MoveX--; } else if (walkDir == 1) { MoveY--; }
	 * else if (walkDir == 2) { MoveX++; MoveY--; } else if (walkDir == 3) {
	 * MoveX--; } else if (walkDir == 4) { MoveX++; } else if (walkDir == 5) {
	 * MoveX--; MoveY++; } else if (walkDir == 6) { MoveY++; } else if (walkDir
	 * == 7) { MoveX++; MoveY++; } }
	 * 
	 * public static byte getWalkDir(int MoveX, int MoveY) { if (MoveX == -1 &&
	 * MoveY == -1) { return 0; } else if (MoveX == 0 && MoveY == -1) { return
	 * 1; } else if (MoveX == 1 && MoveY == -1) { return 2; } else if (MoveX ==
	 * -1 && MoveY == 0) { return 3; } else if (MoveX == 1 && MoveY == 0) {
	 * return 4; } else if (MoveX == -1 && MoveY == 1) { return 5; } else if
	 * (MoveX == 0 && MoveY == 1) { return 6; } else if (MoveX == 1 && MoveY ==
	 * 1) { return 7; } else { System.out.println("Errong walk dir"); return -1;
	 * } }
	 */
	...
	/**
	 * From Ziotic / Dementhium (Mainly Ziotic)
	 * Thanks laz <3
	 */
	...
	public static WalkingDirection directionFor(RSTile currentPos, RSTile nextPos) {
	...
	public static int getNPCWalkDir(int dirX, int dirY) {
		if (dirX < 0) {
			if (dirY < 0)
				return WalkingDirection.SOUTH_WEST.npcIntValue();
			else if (dirY > 0)
				return WalkingDirection.NORTH_WEST.npcIntValue();
			else
				return WalkingDirection.WEST.npcIntValue();
		} else if (dirX > 0) {
			if (dirY < 0)
				return WalkingDirection.SOUTH_EAST.npcIntValue();
			else if (dirY > 0)
				return WalkingDirection.NORTH_EAST.npcIntValue();
			else
				return WalkingDirection.EAST.npcIntValue();
		} else {
			if (dirY < 0)
				return WalkingDirection.SOUTH.npcIntValue();
			else if (dirY > 0)
				return WalkingDirection.NORTH.npcIntValue();
			else
				return -1;
		}
	}
