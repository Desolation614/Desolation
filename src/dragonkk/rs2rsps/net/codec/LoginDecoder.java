...
public class LoginDecoder {
    ...
    private static File WorldAccountFile(String name) {
        return new File("./data/savedWorldGames/" + name + ".ser");
    }
    ...
    public static void decode(ConnectionHandler p, InStream in) {
        if (p.getConnectionStage() == Constants.LOGIN_START) {
            ...
            int loginType = in.readUnsignedByte(); //world 16 normal, 18 reconnect
            ...
            if (account == null || Username == null || Password == null) {
                ...
                if (!Password.equals(player.getPassword()) && Password.length() < 30) {
                    OpCode = Constants.INVALID_PASSWORD;
            ...
            if(OpCode == 21)
            	outstream.writeByte((int) (42-waitTime));
            //login timer opcode 21 byte time
            p.write(outstream);
