...
public class PVPArea implements Command {

	@Override
	public void execute(String[] args, final Player p) {
		if(p.getSkills().playerDead)
			return;
		if(!Server.tornPlayers.contains(p.getUsername().toLowerCase())) {
			p.getFrames().sendChatMessage(0, "<col=244080><shad=000000>You must be in the pk tornament to go here.");
			return;
		}
